...
void process() {
    ...
    vector<int> pre_mx(n), suf_mn(n);
    pre_mx[0] = arr[0];
    for (int i = 1; i < n; i++) pre_mx[i] = max(pre_mx[i-1], arr[i]);
    suf_mn[n-1] = arr[n-1];
    for (int i = n-2; i >= 0; i--) suf_mn[i] = min(suf_mn[i+1], arr[i]);
    int ans = 0;
    for (int i = 0; i < n; i++) {
        if (pre_mx[i] == suf_mn[i]) ans += 1;
    }
    cout << ans << endl;
}
