...
fn main() {
    let (m, n): (usize, usize) = input_helper::scan_line_as_tuple().unwrap();
    let arr: Vec<Vec<char>> = (0..m)
        .map(|_| {
            let s: String = input_helper::scan_line_as_value().unwrap();
            s.chars().collect()
        })
        .collect();

    let mut mem: HashSet<Vec<char>> = HashSet::new();
    for i in 0..=m-n {
        for j in 0..=m-n {
            let subgrid_slice: Vec<char> = arr[i..i+n].iter()
                .flat_map(|row| row[j..j+n].iter().cloned())
                .collect();
            mem.insert(subgrid_slice);
        }
    }
