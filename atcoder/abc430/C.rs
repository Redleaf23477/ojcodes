...
fn main() {
    let (n, a, b): (usize, usize, usize) = input_helper::scan_line_as_tuple().unwrap();
    let s: Vec<usize> = input_helper::scan_line()
        .unwrap()
        .chars()
    ...
    let mut ans: u64 = 0;
    let mut right_a: usize = 0;
    let mut right_b: usize = 0;
    let mut count = [0, 0];

    for left in 0..n {
        // move right_a
        while right_a < n && count[0] < a {
            count[0] += (s[right_a] == 0) as usize;
            right_a += 1;
        }
        if count[0] != a {
            break;
        ...
        // move right_b
        while right_b < n && count[1] < b {
            count[1] += (s[right_b] == 1) as usize;
            right_b += 1;
        }
        // update ans
        if count[1] < b {
            ans += (right_b - right_a + 1) as u64;
            // eprintln!("left={left}, right_b={right_b}, right_a={right_a}");
        } else if right_a < right_b {
            ans += (right_b - right_a) as u64;
            // eprintln!("left={left}, right_b={right_b}, right_a={right_a}");
        }
        // move left
        count[s[left]] -= 1;
