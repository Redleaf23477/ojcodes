...
fn run() {
    let a_str: Vec<char> = input_helper::scan_line().unwrap().chars().collect();
    let b_str: Vec<char> = input_helper::scan_line().unwrap().chars().collect();
    let b_aa_str = {
        let mut b_aa_str = Vec::with_capacity(b_str.len() + 1 + 2 * a_str.len());
        b_aa_str.extend(&b_str);
        b_aa_str.push('$');
        b_aa_str.extend(&a_str);
        b_aa_str.extend(&a_str);
        b_aa_str
    };
    let kmp = run_kmp(&b_aa_str);

    let ans = (a_str.len() + 1..b_aa_str.len())
        .zip(&kmp[a_str.len() + 1..])
        .find(|&(_, &lps)| lps == b_str.len())
        .map(|(i, _)| (i - 2 * b_str.len()) as i32)
        .unwrap_or(-1);
    /*
    let mut ans = -1 as i32;
    for i in a_str.len() + 1 .. b_aa_str.len() {
        if kmp[i] == b_str.len() {
            ans = (i - b_str.len() - b_str.len()) as i32;
            break;
        }
    }
     */
...
fn run_kmp(s: &Vec<char>) -> Vec<usize> {
    let mut kmp = vec![0; s.len()];
    kmp[0] = 0;

    for i in 1..s.len() {
        let mut cursor = kmp[i - 1];
        while s[i] != s[cursor] && cursor != 0 {
            cursor = kmp[cursor - 1];
        }
        if s[i] == s[cursor] {
            cursor += 1;
