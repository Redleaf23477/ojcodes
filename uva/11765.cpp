...
void init()
{
    ...
    for(int i = 0; i < n; i++)
    {
        int layer; cin >> layer;
        if(layer == 0)
            dinic.add_edge(s, i, x[i]), dinic.add_edge(i, t, y[i]);
        else if(layer == 1)
            dinic.add_edge(s, i, x[i]), dinic.add_edge(i, t, INF);
        else
            dinic.add_edge(s, i, INF), dinic.add_edge(i, t, y[i]);
    }
    for(int i = 0; i < m; i++)
    {
        int u, v; ll w; cin >> u >> v >> w;
        u--, v--;
        dinic.add_edge(u, v, w), dinic.add_edge(v, u, w);
    }
}
